...
#theory spatial {
	...
	&simulation/1 : spatial_term, any
}.
"""


theory_atom_names = ["union", "intersect", "diff", "buffer", "topology", "draw", "simulation"]
		...
		for _ in range(init.number_of_threads):
			...
			with checker.backend() as backend:
				...
				for atom in init.theory_atoms:
					...
					if op == "simulation" and len(term.arguments) == 1:

						#print(atom)
						ids = [atom.elements[0].terms[i] for i in range(0, 6)]

						id = ids[0]
						
						new_id = str([id.number for id in ids])
	...
	def _evaluate(self, head_atoms):
		...
		for h in head_atoms:
			if not self._in(h[2], heads): 
				...
				if h[0] == "simulation":
					#print(h)
					args = ast.literal_eval(h[2])
					#print(args)
					geoms = [args[0], args[1], args[2], args[3], args[4], args[5]]
					geoms = [str(i) for i in geoms]
					new_id = 'simulation(' + ','.join(geoms) + ')'
					#print(new_id)
					if new_id not in self._geoms.keys():
						print(new_id)
						self._geoms.setdefault(new_id, 0)
					#self._assigns.setdefault(res, new_id)
					#print(type(args[0]))
